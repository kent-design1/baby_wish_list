...
export default function PlasmaWave(props: PlasmaWaveProps) {
    const {
        xOffset = 0, yOffset = 0, rotationDeg = 0,
        focalLength = 0.8, speed1 = 0.05, speed2 = 0.05,
        dir2 = 1.0, bend1 = 1, bend2 = 0.5,
    ...
    useEffect(() => {
        ...
        const renderer = new Renderer({
            alpha: true, dpr: Math.min(window.devicePixelRatio, 1.5),
        ...
        const update = (now: number) => {
            ...
            program.uniforms.uDir2.value = p.dir2 ?? 1.0;
            program.uniforms.uBend1.value = p.bend1 ?? 1;
            program.uniforms.uBend2.value = p.bend2 ?? 0.5;
            program.uniforms.uColor1.value = hexToRgb((p.colors ?? ["#A855F7", "#06B6D4"])[0]);
            program.uniforms.uColor2.value = hexToRgb((p.colors ?? ["#A855F7", "#06B6D4"])[1]);
